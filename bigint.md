...
## 引言
在 [integer-fields.md](integer-fields.md#utf8-int) 中，我们讲述了把 **Unicode 码点**
...
{i{min,max}}
```
这个语法足够简单，整数用十进制表达，min,max 可以为负数，也可以为空，为空时表示 <span style="font-size: 2em;">∞</span> (无穷)：

`{i{min,max}}` | 闭区间 `[min,max]`
---------------|:----------------------------------
`{i{,max}}`  | 区间 (-<span style="font-size: 2em;">∞</span>, max] ，表示所有 **小于等于** max 的整数，max 可以为负数
`{i{min,}}`  | 区间 [min, +<span style="font-size: 2em;">∞</span>) ，表示所有 **大于等于** min 的整数，min 可以为负数
`{i{,}}`     | 区间 (-<span style="font-size: 2em;">∞</span>, +<span style="font-size: 2em;">∞</span>) ，表示所有所有整数，包含所有正负数和 0
...
## 复杂点的例子
> RuleDB 支持[联合索引](realnum.md#lat-longt)，此处仅作为原理性展示

我们仍以 [integer-fields.md](integer-fields.md#utf8-int) 中的 gender_age_income 为例：
```bash
