...
\begin{titlepage}
    \centering
    \vspace*{2cm} % * 强制在段落开头插入 vspace
    % \vfill % 垂直居中，试过了不管用
    \includegraphics[width=6.4cm, keepaspectratio=true]{logo.pdf}
    \\ % 换行
    \vspace{1.5cm}  % Logo 与标题的间距
    ...
    \vspace{1cm}
    $if(url)$
      {\normalsize Homepage: $url$\par}
    $endif$
    $if(email)$
      {\normalsize email: $email$\par}
    $endif$
\end{titlepage}
$endif$
$if(abstract)$
