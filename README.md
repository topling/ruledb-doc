# 简介
Topling 规则数据库的功能是：将 $$\color{#DC143C}{预定义规则}$$ 作为数据库，将 $$\color{#DC143C}{输入文本}$$ 作为查询词，搜索预定义的规则。
## 规则表达式
...
其中 num 的单位是：对于中文，每个中文字符为1，对于英文/数字，每个英文单词/数字为1。例如 computer , 10086 , varname_123 均视为 1 个距离单位。VeryBigComputerLongWord9558493 也视为 1 个距离单位。计算距离时会排除特殊字符和标点符号、空格等。
