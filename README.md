...
高效地支持 [数值](integer-fields.md) 与 [geo空间数据](geo-fields.md)，以及不限宽度的[整数(范围)](bigint.md)，[实数(范围)](realnum.md)，支持[联合索引](realnum.md#lat-longt)。
...
`{i{min,max}}` 是[大整数](bigint.md)区间的语法（和 [ ] 一样在**非全匹配**时默认禁用，主要用于**全匹配**）。
