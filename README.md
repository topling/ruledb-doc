# <a name="quick-start"></a> RuleDB 快速入门
## 概述
Topling RuleDB 规则数据库的功能是：将 **预定义规则** 作为数据库，将 **输入文档** 作为查询条件，搜索预定义的规则。相比同类产品有 100 倍以上的性能优势。

应用场景包括：敏感词检测、广告定向、数据分类、事件分流、自动打标签等，比现存其它方案
(例如drools/ElasticSearch percolator等)性能高至少两个数量级，规则越多，优势越突出，支持
百万条以上的规则，每条规则的表达式可以任意复杂。表达式的原生数据类型是字符串，也能
高效地支持 [utf8数值](integer-fields.md#utf8-int) 与 [geo空间数据](geo-fields.md#geo-field)，以及不限宽度的[整数(范围)](bigint.md#bigint)，[实数(范围)](realnum.md#realnum)，支持[联合索引](realnum.md#lat-longt)。
...
例子                |  等价写法        | 含义
--------------------|----------------|-------------------------------------------
`中国 and 人民`      | `中国 & 人民`    | 既包含`中国`又包含`人民`
`中国 or 人民`       | `中国 \| 人民`    | 包含`中国`或包含`人民`
||
`中国 and not 日本`  | `中国 &! 日本`   | 包含 `中国`不包含`日本`
`中国 not 日本`      | `中国 - 日本`    | 包含 `中国`不包含`日本`
...
fieldname[..表达式..] 表示仅在 fieldname 字段上检查该表达式，匹配该字段的全部内容。**全匹配**支持多字段[联合索引](realnum.md#lat-longt)，拥有完备的多维空间搜索功能。
...
```bash
(中国|中华|华夏) near/3 ((人民|百姓) near/5 (勤劳|勤奋) | (历史悠久|幅员辽阔) & 地大物博)
```

near 的子表达式可以是 or 表达式，如果还包含 near,and,not 等子表达式，会通过分配率提到外层。
这种 near 是对其作为正则语言的语义的少许(人类难以察觉的)弱化以保证性能。

## 正则表达式
原子查询词可以是正则表达式，简单的正则表达式用 {{....}} 括起来，例如：

`{{(\(010\)|010)-?{[0-9]{8}}}}` 表示北京的电话号码。

## <a name="复合正则表达式"></a> 复合正则表达式

用 [ ... ] 表达括起来的是 **复合正则表达式**，例如：
...
默认情况下**非全匹配**表达式中该功能**复合正则表达式**是禁用的，如要启用需设置环境变量 `RULEDB_USE_COMPOUND_REGEX=1`。

在**全匹配**表达式中该功能**复合正则表达式**总是启用的。

对于指定字段的场景，[ ] 内只包含一个正则表达式时，`fieldname[{{ ...表达式... }}]` **全匹配**可简写为 `fieldname{{ ...表达式... }}`。

### [ ] 内的 near
默认情况下该功能是禁用的，如要启用需设置环境变量 `RULEDB_USE_COMPOUND_REGEX=1`。
...
复合正则表达式支持串接操作 `.` ，类似字符串的直接串接，在多维搜索的场景下等效于笛卡尔积(参考[大整数](bigint.md#bigint))，例如：
...
`{i{min,max}}` 是[大整数](bigint.md#bigint)区间的语法（和 [ ] 一样在**非全匹配**时默认禁用，主要用于**全匹配**）。
...
## 频次表达式
```bash
中国 == {3} and 人民 >= {5} # "中国" 的频次 == 3 并且"人民"的频次 >= 5
中国 ==     人民  # "中国" 的频次 == "人民"
中国 >*1.8  日本  # "中国" 的频次 >  "日本" 的 1.8 倍
中国 >=*1.4 美国  # "中国" 的频次 >= "美国" 的 1.4 倍
美食 <=*2.8 饭馆  # "美食" 的频次 <= "饭馆" 的 2.8 倍
中国 == 人民 or 勤劳 != 勇敢
```

**词项频次与常量比较时，词项表达式必须在左边：**
```bash
...
```bash
(中国|中华|华夏|china) == {,8}
(中国|中华|华夏|china) == {8}  # 8 是频次, 等价于{,8}
```

词项列表(一般是同义词集合)的**互相比较**，仅比较 freq，不比较 uniq:
```bash
(中国|中华|华夏|china) >= (日本|扶桑|japan)
```

near 序列的**互相比较**:
```bash
中国 near/2 崛起 near/3 必然 >= 中国 near/5 落后
```

near 链式比较，如果有常量 {freq}，只能位于末尾：
```bash
中国 >= 美国 >= 日本 >= 韩国
中国 >= 美国 >= 日本 >= {1}
# 等效于
中国 >= 美国 and 美国 >= 日本 and 日本 >= {1}
```
...
------| ------|------------------------------------------------------
-h    | 无参数   | 打印帮助信息
-i    | 联合索引 | s{sepa}(field1,field2,...)，其中 s{sepa} 是可选的，省略时 sepa 是 `\0`, [示例用法](realnum.md#lat-longt)
...
```
### 4.3. <a name="43-json-字符串作为-doc"></a> Json 字符串作为 doc
```c++
