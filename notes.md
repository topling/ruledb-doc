...
从而如果一个词本身虽然是高频词，但只出现在少量规则中，对性能的影响就不大。

值得注意的是，RuleDB 最新版使用了启发式的迭代算法对召回进行了优化，大部分情况下都可以达到很好的效果，所以一般情况下业务方无需对此进行过早优化。

## 2. [ ] 中的 near

为了保证性能，**该功能默认是关闭的**，要开启该功能，需要设置环境变量 `RULEDB_USE_COMPOUND_REGEX=1`。
